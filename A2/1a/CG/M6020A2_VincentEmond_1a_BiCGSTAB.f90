...
!call matrix vector product subroutine
...
p=p
call amuxd(n,p,v,a,ndiag,ioff)
alfa=rho/(dot_product(rtil,v))
s=r-alfa*v

   call amuxd(n,s,t,a,ndiag,ioff)
   omega=dot_product(t,s)/dot_product(t,t)
   sol=sol+alfa*p+omega*s
   r=s-omega*t

  !check norm of s, exit if s is too large
  if (sqrt(sum(s*s))/n<tol1) then
  exit
  end if  
